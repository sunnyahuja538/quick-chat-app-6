...
const ChatContainer = () => {
  const {messages,selectedUser,setSelectedUser,sendMessage,getMessages}=useContext(ChatDataContext);
  ...
  useEffect(()=>{
   ...
   useEffect(()=>{
    ...
    },[messages])//it calls scrollIntoView(...) → a native DOM API that automatically scrolls the page so that element is visible.
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <img  src={assets.help_icon} className='md:hidden max-w-5'/>
      </div>
      {/* chat area */}
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg,index)=>{
          ...
          <>
            {console.log("hello")}
            <p className={`p-3 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${msg.senderId!==authUser._id?'rounded-br-none':'rounded-bl-none'}`}>{msg.text}</p>
            </>)}
            <div className='text-center text-xs'>
              <img src={msg.senderId===authUser._id?assets.avatar_icon:assets.profile_martin} className='w-7 rounded-full'/>
